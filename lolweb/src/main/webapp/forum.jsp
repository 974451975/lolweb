...
<head>
    ...
    <!-- 富文本编辑器配置文件 -->
    <script type="text/javascript" charset="utf-8" src="${applicationScope.basePath}ueditor/ueditor.config.js"></script>
    <!-- 读取富文本编辑器配置的函数脚本 -->
    <script type="text/javascript" charset="utf-8" src="${applicationScope.basePath}ueditor/ueditor.all.min.js">
    </script>
    <!-- 支持中文编码 -->
    <script type="text/javascript" charset="utf-8" src="${applicationScope.basePath}ueditor/lang/zh-cn/zh-cn.js"></script>
...
<div align="center">
    <div style="width: 80%;">
        <!--楼主发帖层-->
        <div>
            <h2>${requestScope.forum.forum_title}</h2>
            <button onclick="gotohis()" style="float: left">回上一页</button>
        <c:if test="${empty sessionScope.loginuser}">
            <a href="#mymodal" data-toggle="modal" style="float: right;margin: 10px;">请先登录在使用收藏功能</a>
        </c:if>

           <c:if test="${not empty sessionScope.loginuser}">
               <c:if test="${requestScope.isCollect==true}">
                   <input type="button" style="float: right;margin: 10px;" value="取消收藏" onclick="iscollect(0)">
               </c:if>
               <c:if test="${requestScope.isCollect==false}">
                   <input type="button" style="float: right;margin: 10px;" value="收藏此贴" onclick="iscollect(1)">
               </c:if>
           </c:if>

            <script>
                function iscollect(op) {
                    if(op==1){
                        window.location.href="addCollect.do?forumid=${requestScope.forum.forum_id}&userid=${sessionScope.loginuser.user_id}"
                    }
                    else{
                        window.location.href="cancelCollect.do?forumid=${requestScope.forum.forum_id}&userid=${sessionScope.loginuser.user_id}"
                    }
                }
            ...
            <div style="border: 1px solid #927345; padding: 30px; background-color: rgba(200,255,200,0.1);" align="left">
                <!--头像栏-->
                <div style="float: left;border-right:1px solid #B3696C; margin-right: 10px;">
                    <div style="margin: 10px 20px;"><img src="../upload/${requestScope.forum.user_img}" style="width: 100px ;height: 100px; border: 1px solid #927345;"/>
                        <div align="center">
                            <div style="margin-top: 10px ">${requestScope.forum.user_name}</div>
                            <div>LV:${requestScope.forum.user_level}</div>
                        </div>
                    </div>
                </div>
                <!--文本栏-->
                <p style="margin-top: 10px;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;${requestScope.forum.forum_text}
                </p>
                <div style="margin-top: 5%;float: right;"><span>——————data:${requestScope.forum.forum_time}</span></div>
        ...
        <script>
            ...
            $(document).ready(mysearch(1))

            function mysearch(op){

                $.getJSON("replyforpage.do", {page: op,forumid:${requestScope.forum.forum_id}}, function (data) {


                    var i = 0;
                    var str = ""
                    for (i; i < data.list.length; i++) {
                        str=str+"<li><div style=' padding: 30px;' align='left'>"+
                            "<div style='float: left;border-right:1px solid #B3696C; margin-right: 10px;'>"+
                            "<div style='margin: 10px 20px'><img src='../upload/"+data.list[i].user_img+"' style='width: 100px ;height: 100px; border: 1px solid #927345;'/>"+
                            "<div align='center'><div style='margin-top: 10px'>"+data.list[i].user_name+"</div>"+
                            "<div>LV:"+data.list[i].user_level+"</div>"+
                            "</div></div></div>"+
                            "<p style='margin-top: 10px;'>"+data.list[i].reply_text+"</p>"+
                            "<div style='margin-top: 5%;float: right;\'><span>——————data:"+data.list[i].reply_time+"</span></div>"+
                            "<div style='clear: both;'></div>"+
                            "</div><hr style='clear: both;' /></li>"
                    }
                    $("#showreply").html(str);
                    var page=data.page;
                    ...
                    for(i;i<=maxpage;i++){
                        if(i==page){
                            ...
                            str1=str1+"<a href='javascript:mysearch("+i+")'>第"+i+"页</a>&nbsp;&nbsp;&nbsp;&nbsp;"
                        }
                    }
                    $("#mypage").html(str1);
        ...
        <ul style="list-style: none;" id="showreply">
            <%--<li>--%>
                <%--<div style=" padding: 30px;" align="left">--%>
                    <%--<!--头像栏-->--%>
                    <%--<div style="float: left;border-right:1px solid #B3696C; margin-right: 10px;">--%>
                        <%--<div style="margin: 10px 20px;"><img src="images/g7big.jpg" style="width: 100px ;height: 100px; border: 1px solid #927345;"/>--%>
                            <%--<div align="center">--%>
                                <%--<div style="margin-top: 10px ">LOL小美</div>--%>
                                <%--<div>LV:99</div>--%>
                            <%--</div>--%>
                        <%--</div>--%>
                    <%--</div>--%>
                    <%--<!--文本栏-->--%>
                    <%--<p style="margin-top: 10px;">说的好啊！！！ </p>--%>
                    <%--<div style="margin-top: 5%;float: right;"><span>——————data:2020/10/10 18:00:01</span></div>--%>
                    <%--<div style="clear: both;"></div>--%>
                <%--</div>--%>
                <%--<hr style="clear: both;" />--%>
            <%--</li>--%>
        ...
        <div align="center" style="clear: both" id="mypage">

        </div>
        <!--回帖区-->
        <div style="">
            <c:if test="${empty sessionScope.loginuser}">
                <p style="margin:50px auto"><a href="#mymodal" data-toggle="modal" >回帖请先登录</a></p>
            </c:if>
            <c:if test="${not empty sessionScope.loginuser}">
            <div class="rj_content" align="center">
                <form action="replyForum.do?userid=${sessionScope.loginuser.user_id}&forumid=${requestScope.forum.forum_id}" method="post" enctype="multipart/form-data">
                    <div style="" align="left">
                        <p>回复此贴:</p>
                    </div>
                    <script id="editor" name="replytext" type="text/plain" style="width:90%;height:300px;"></script>
