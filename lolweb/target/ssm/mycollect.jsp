...
<script>
    ...
    function queryforum(op) {
        window.location.href="forumcontroller/queryForumById.do?forumid="+op;
    }

    function cancelcollect(op) {
        window.location.href="forumcontroller/cancelCollect.do?userid=${sessionScope.loginuser.user_id}&forumid="+op;
    }
...
<script>
    $(document).ready(function(){
        $.getJSON("forumcontroller/queryCollectByUserId.do?userid=${sessionScope.loginuser.user_id}&page=1",function(data){
            if(data!=null) {
                var str="";
                for(var i=0;i<data.length;i++){
                    str=str+" <li style='font-size: 16px;padding-top: 20px;border: 1px solid #8A6D3B;height: 150px;margin-top: 4%;'>"+
                            "<h4>"+data[i].forum_title+"</h4>"+
                            "<div>收藏时间："+data[i].collect_time+"</div>"+
                            "<input type='button' value='查看该贴' onclick='queryforum("+data[i].forum_id+")'/>" +
                            "<input type='button' value='取消收藏' onclick='cancelcollect("+data[i].forum_id+")' /></li>"

                }
                $("#showcollect").html(str);
                }
            });
    });
...
<div>
    ...
    <div style="width: 80%; margin: auto; " align="center">
        <div style="width: 100%;">
            <ul style="list-style:none;" id="showcollect">
